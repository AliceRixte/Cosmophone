#include "Cosmophone.h"
#include "IPlug_include_in_plug_src.h"
#include "LFO.h"

Cosmophone::Cosmophone(const InstanceInfo& info)
...
#if IPLUG_DSP
void Cosmophone::ProcessBlock(sample** inputs, sample** outputs, int nFrames)
{
...
void Cosmophone::OnIdle()
{
  mMeterSender.TransmitData(*this);
  mLFOVisSender.TransmitData(*this);
}

void Cosmophone::OnReset()
{
  mDSP.Reset(GetSampleRate(), GetBlockSize());
  mMeterSender.Reset(GetSampleRate());
}

void Cosmophone::ProcessMidiMsg(const IMidiMsg& msg)
{
  TRACE;
...
void Cosmophone::OnParamChange(int paramIdx)
{
  mDSP.SetParam(paramIdx, GetParam(paramIdx)->Value());
}

bool Cosmophone::OnMessage(int msgTag, int ctrlTag, int dataSize, const void* pData)
{
