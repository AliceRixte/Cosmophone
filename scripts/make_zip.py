...
def  main():
  ...
  installer = "\installer\Cosmophone Installer.exe"
   
  if demo:
    installer = "\installer\Cosmophone Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Cosmophone manual.pdf" 
  ]
  ...
  ZIPNAME = "Cosmophone-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Cosmophone-v" + FULLVERSIONSTR + "-win-demo.zip"
